...
class ImportBmeVigor extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importVigor';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeVigor.xml', 0, TRUE);
        $counter = 0;
                    ...
                    if ($list->getEan() == $xmlEan) {
                        ...
                        foreach ($result->MIME_INFO as $articleImages) {
                            foreach ($articleImages as $articleImage) {
                                if ($articleImage->MIME_TYPE == "image/jpeg" && str_contains($articleImage->MIME_SOURCE, "Produktbilder")) {
