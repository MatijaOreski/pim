...
class ImportBmeKnipex extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importKnipex';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeKnipex.xml', 0, TRUE);
        $counter = 0;
                ...
                foreach ($eanList as $list) {
                    if ($list->getEan() == $xmlEan) {
                        ...
                        foreach ($result->PRODUCT_FEATURES as $items){
                            if($items->REFERENCE_FEATURE_GROUP_NAME == "PRODUCT"){
                                foreach ($items->FEATURE as $item){
                                    if($item->FNAME != "Artikelnummer"){
                        ...
                        foreach ($result->PRODUCT_FEATURES as $productPackages) {
                            if($productPackages->REFERENCE_FEATURE_GROUP_NAME == "PRODUCT"){
                                foreach ($productPackages->FEATURE as $productPackage){
                                    if($productPackage->FNAME != "Artikelnummer"){
                                        $blockElementsTechnical[] = [
                        ...
                        foreach ($result->MIME_INFO as $articleImages) {
                            foreach ($articleImages as $articleImage) {
                                if ($articleImage->MIME_TYPE == "image/jpeg" && $articleImage->MIME_PURPOSE == "detail") {
