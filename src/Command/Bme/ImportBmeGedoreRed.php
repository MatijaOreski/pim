...
class ImportBmeGedoreRed extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importGedoreRed';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeGedoreRed.xml', 0, TRUE);
        $counter = 0;
                    ...
                    if ($list->getEan() == $xmlEan) {
                        ...
                        foreach ($result->ARTICLE_FEATURES as $productFeatures){
                            foreach ($productFeatures as $productFeature){
                                if(str_contains($productFeature->FVALUE, "mm") || str_contains($productFeature->FVALUE, "kg") || str_contains($productFeature->FVALUE, "tlg") || str_contains($productFeature->FNAME, "[t]")){
                                    $technicalData .= "<li>" . $productFeature->FNAME . " " . $productFeature->FVALUE . "</li>";
                                }else{
                                    $others .= "<li>" . $productFeature->FNAME . " " . $productFeature->FVALUE . "</li>";
                                }
                            }
                        }
                        $technicalData .= "</ul>";
                        ...
                        foreach ($result->ARTICLE_FEATURES as $productPackages) {
                            foreach ($productPackages as $productPackage) {
                                ...
                                if ($productPackage->FNAME == "Netto-Gewicht [kg]") {
                                    $nettoValue = $productPackage->FVALUE;
                                    $nettoWeight = chop($nettoValue," kg");
                                }
                        ...
                        $checkFirstSixCharacters = substr($shortDescriptionVar, 0, 9);
                        if($checkFirstSixCharacters == "GedoreRed"){
                            $shortDescriptionVar = substr($shortDescriptionVar, 9);
                            $shortDescriptionVar = "GedoreRed " . $shortDescriptionVar;
                        }else{
                            $shortDescriptionVar = "GedoreRed " . $shortDescriptionVar;
                        }
                        ...
                        foreach ($result->ARTICLE_FEATURES as $articleFeatures) {
                            ...
                            foreach ($articleFeatures as $articleFeature) {
                                if (str_contains($articleFeature->FVALUE, "mm") || str_contains($articleFeature->FVALUE, "kg") || str_contains($articleFeature->FVALUE, "tlg") || str_contains($articleFeature->FNAME, "[t]")) {
                                    $blockElementsTechnical[] = [
                        ...
                        foreach ($result->MIME_INFO as $articleImages) {
                            foreach ($articleImages as $articleImage) {
                                if ($articleImage->MIME_TYPE == "image/jpeg" && $articleImage->DESCR == "Katalogbild") {
