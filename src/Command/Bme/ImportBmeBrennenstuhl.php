...
class ImportBmeBrennenstuhl extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importBrennenstuhl';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeBrennenstuhl.xml', 0, TRUE);
        $counter = 0;
            ...
            if ($counter <= 10) {
                ...
                foreach ($eanList as $list) {
                    if ($list->getEan() == $xmlEan) {
                        ...
                        $technicalData .= "<p><strong>Produkt:</strong></p>";
                        $technicalData .= "<ul>";
                        foreach ($result->PRODUCT_DETAILS->BULLETPOINTS->BULLETPOINT as $productPackages){
                            $technicalData .= "<li>" . $productPackages . "</li>";
                        }
                        $technicalData .= "</ul>";
                        ...
                        foreach ($result->MIME_INFO as $articleImages) {
                            foreach ($articleImages as $articleImage) {
                                if ($articleImage->MIME_TYPE == "image/jpeg" && $articleImage->MIME_PURPOSE == "normal" && $articleImage->MIME_DESCR == "Produktbild") {
