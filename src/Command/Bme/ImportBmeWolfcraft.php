...
class ImportBmeWolfcraft extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importWolfcraft';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeWolfcraft.xml', 0, TRUE);
        $counter = 0;

        foreach ($xml->T_NEW_CATALOG->ARTICLE as $result) {
            if ($counter <= 10) {

                //echo $xmlTechnicalData = html_entity_decode($result->ARTICLE_DETAILS->DESCRIPTION_LONG);
                ...
                foreach ($eanList as $list) {
                    if ($list->getEan() == $xmlEan) {
                        ...
                        // description part
                        $xmlTechnicalData = str_replace("H1", "H2", $result->ARTICLE_DETAILS->DESCRIPTION_LONG);
                        $xmlTechnicalData = html_entity_decode($result->ARTICLE_DETAILS->DESCRIPTION_LONG);

                        $technicalData = null;
                        if($result->ARTICLE_FEATURES->FEATURE->FNAME != null){
                            $technicalData .= "<p><strong>Technische Daten:</strong></p>";
                            $technicalData .= "<ul>";
                            foreach ($result->ARTICLE_FEATURES->FEATURE as $feature){
                                $technicalData .= "<li>" . $feature->FNAME. " " . $feature->FVALUE . "</li>";
                            }
                            $technicalData .= "</ul>";
                        }
                        ...
                        $articleFeaturesList = [];
                        foreach ($result->ARTICLE_FEATURES->FEATURE as $articleFeature) {

                            $featureValue = preg_replace('/[^0-9.]+/', '', $articleFeature->FVALUE);
                            if($featureValue){
                                $featureUnit = preg_replace("/[^a-zA-Z]+/", "", $articleFeature->FVALUE);
                            }

                            $blockElementsTechnical[] = [
                                "name" => new BlockElement('name', 'input', strval($articleFeature->FNAME)),
                                "val" => new BlockElement('val', 'input', strval($articleFeature->FVALUE)),
                                "unit" => new BlockElement('unit', 'input', strval($featureUnit))
                            ];
                        }
                        ...
                        foreach ($result->MIME_INFO as $articleImages) {
                            foreach ($articleImages as $articleImage) {
                                ...
                                if ($articleImage->MIME_TYPE == "image/jpeg" && $articleImage->MIME_MIME_PURPOSE == "normal") {
