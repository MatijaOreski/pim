...
class ImportBmeGude extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importGude';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeGude.xml', 0, TRUE);
        $counter = 0;
            ...
            if ($counter <= 10) {
                ...
                foreach ($eanList as $list) {
                    if ($list->getEan() == $xmlEan) {
                        ...
                        foreach ($result->ARTICLE_FEATURES->FEATURE as $productPackages) {
                            if($productPackages->FNAME == "Bruttogewicht"){
                                $bruttoWeight = $productPackages->FVALUE;
                            }
                            if($productPackages->FNAME == "Nettogewicht"){
                                $nettoWeight = $productPackages->FVALUE;
                            }
                            if($productPackages->FNAME == "Länge"){
                                $dimensionLength = $productPackages->FVALUE;
                            }
                            if($productPackages->FNAME == "Breite"){
                                $dimensionWidth = $productPackages->FVALUE;
                            }
                            if($productPackages->FNAME == "Höhe"){
                                $dimensionHeight = $productPackages->FVALUE;
                            }
                        }
                        ...
                        foreach ($result->ARTICLE_FEATURES->FEATURE as $articleFeatures) {
                            if($articleFeatures->FNAME != "GTIN"){
                                if($articleFeatures->FNAME != "Artikelnummer"){
                                    $articleFeaturesList = [];
                                    $blockElementsTechnical[] = [
                        ...
                        foreach ($result->MIME_INFO as $articleImages) {
                            foreach ($articleImages as $articleImage) {
                                if ($articleImage->MIME_TYPE == "image/jpeg" && $articleImage->MIME_DESCR == "normal" && $articleImage->MIME_PURPOSE == "normal") {
