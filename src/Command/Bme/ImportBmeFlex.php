...
class ImportBmeFlex extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importFlex';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeFlex.xml', 0, TRUE);
        $counter = 0;
                ...
                foreach ($eanList as $list) {
                    if ($list->getEan() == $xmlEan) {
                        ...
                        foreach ($result->PRODUCT_FEATURES as $items){
                            if($items->REFERENCE_FEATURE_GROUP_ID == "PIM"){
                        ...
                        if($checkFirstSixCharacters == "FLEX"){
                            $shortDescriptionVar = substr($shortDescriptionVar, 4);
                            $shortDescriptionVar = "Flex " . $shortDescriptionVar;
                        }else{
                            $shortDescriptionVar = "Flex " . $shortDescriptionVar;
                        }
                        ...
                        foreach ($result->PRODUCT_FEATURES as $productPackages) {
                            if($productPackages->REFERENCE_FEATURE_GROUP_ID == "PIM"){
                        ...
                        $udx_edxf_mime_designation = "UDX.EDXF.MIME_DESIGNATION";
                        ...
                        foreach ($result->USER_DEFINED_EXTENSIONS->$udx_edxf_mime_info->$udx_edxf_mime as $articleImage) {
                            if ($articleImage->$udx_edxf_mime_designation == "Produktbild") {
