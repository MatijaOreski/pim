...
class ImportBmeWera extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importWera';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeWera.xml', 0, TRUE);
        $counter = 0;
                    ...
                    if ($list->getEan() == $xmlEan) {
                        ...
                        foreach ($result->MIME_INFO as $articleImages) {
                            foreach ($articleImages as $articleImage) {
                                if ($articleImage->MIME_TYPE == "image/jpeg" && $articleImage->MIME_PURPOSE == "detail" && $articleImage->MIME_ORDER == "1") {
