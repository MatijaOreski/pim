...
class ImportBmeFestool extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importFestool';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeFestool.xml', 0, TRUE);
        $counter = 0;
        ...
        foreach ($xml->T_NEW_CATALOG->ARTICLE as $result) {
            ...
            foreach ($eanList as $list) {
                if ($list->getEan() == $xmlEan) {
                    ...
                    // description
                    $xmlTechnicalData = substr($result->ARTICLE_DETAILS->DESCRIPTION_LONG, 0, strpos($result->ARTICLE_DETAILS->DESCRIPTION_LONG, "Garantie"));
                    //$xmlTechnicalData = "<p>" . $xmlTechnicalData . "</p>";

                    $technicalData = null;
                    if (str_contains($result->ARTICLE_FEATURES->FEATURE->FDESCR, "Technische Daten")) {
                        $technicalData .= "<p><strong>" . strtok($result->ARTICLE_FEATURES->FEATURE->FDESCR, "/") . "</strong></p>";
                        $technicalData .= "<ul>";
                        foreach ($result->ARTICLE_FEATURES->FEATURE as $technicalFeatures) {
                    ...
                    if($checkFirstSixCharacters == "Festool"){
                        $shortDescriptionVar = substr($shortDescriptionVar, 7);
                        $shortDescriptionVar = "Festool " . $shortDescriptionVar;
                    }else{
                        $shortDescriptionVar = "Festool " . $shortDescriptionVar;
                    }
                    ...
                    foreach ($result->ARTICLE_FEATURES as $articleFeatures) {
                        foreach ($articleFeatures as $articleFeature) {
                            if (str_contains($articleFeature->FDESCR, "Technische Daten")) {
                                $blockElementsTechnical[] = [
                    ...
                    foreach ($result->MIME_INFO as $articleImages) {
                        foreach ($articleImages as $articleImage) {
                            if ($articleImage->MIME_DESCR == "Produktbild" && $articleImage->MIME_PURPOSE == "normal") {
                                $mainImage = $articleImage->MIME_SOURCE;
                                array_push($galleryImage, $mainImage);
                            }
                            if ($articleImage->MIME_TYPE == "image/jpeg" && $articleImage->MIME_ORDER != "1") {
