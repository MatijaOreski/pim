...
class ImportBmeFein extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importFein';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeFein.xml', 0, TRUE);
        $counter = 0;
            ...
            if ($counter == 0) {
                ...
                foreach ($eanList as $list) {
                    if ($list->getEan() == $xmlEan) {
                        ...
                        // description part
                        $xmlTechnicalData = "<p>" . str_replace("#8226;", "nbsp;", $result->ARTICLE_DETAILS->DESCRIPTION_LONG) . "</p>";
                        ...
                        foreach ($result->ARTICLE_FEATURES as $productPackages){
                            foreach ($productPackages as $productPackage){
                                if(str_contains($productPackage->FNAME, "TariffNo") || str_contains($productPackage->FNAME, "DisGroup") || str_contains($productPackage->FNAME, "PackagingLength") || str_contains($productPackage->FNAME, "PackagingWidth") || str_contains($productPackage->FNAME, "PackagingHeight") || str_contains($productPackage->FNAME, "GrossWeight") || str_contains($productPackage->FNAME, "CountryOfOrigin") || str_contains($productPackage->FNAME, "Top-Features") || str_contains($productPackage->FNAME, "FEIN Vorteile")){
                                    $others = "-" . $productPackage->FNAME . " " . $productPackage->FVALUE . " " . $productPackage->FUNIT . "\n";
                                }else{
                                    if($productPackage->FNAME == ""){
                                        $nothing = null;
                                    }else{
                                        $technicalData .= "<li>" . $productPackage->FNAME . " " . $productPackage->FVALUE . " " . $productPackage->FUNIT . "</li>";
                                    }
                        ...
                        foreach ($result->ARTICLE_FEATURES as $productPackages) {
                            foreach ($productPackages as $productPackage) {
                                ...
                                if ($productPackage->FNAME == "GrossWeight") {
                                    $nettoWeight = $productPackage->FVALUE;
                                }
                                if ($productPackage->FNAME == "PackagingLength") {
                        ...
                        $checkFirstSixCharacters = substr($shortDescriptionVar, 0, 4);
                        if($checkFirstSixCharacters == "FEIN"){
                            $shortDescriptionVar = substr($shortDescriptionVar, 4);
                            $shortDescriptionVar = "Fein " . $shortDescriptionVar;
                        }else{
                            $shortDescriptionVar = "Fein " . $shortDescriptionVar;
                        }
                        ...
                        foreach ($result->ARTICLE_FEATURES as $articleFeatures) {
                            $articleFeaturesList = [];
                            foreach ($articleFeatures as $productPackage) {
                                if (str_contains($productPackage->FNAME, "TariffNo") || str_contains($productPackage->FNAME, "DisGroup") || str_contains($productPackage->FNAME, "PackagingLength") || str_contains($productPackage->FNAME, "PackagingWidth") || str_contains($productPackage->FNAME, "PackagingHeight") || str_contains($productPackage->FNAME, "GrossWeight") || str_contains($productPackage->FNAME, "CountryOfOrigin") || str_contains($productPackage->FNAME, "Top-Features") || str_contains($productPackage->FNAME, "FEIN Vorteile")) {
                                    $others = $productPackage->FNAME . " " . $productPackage->FVALUE . " " . $productPackage->FUNIT;
