...
class ImportBmeFischer extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importFischer';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeFischer.xml', 0, TRUE);
        $counter = 0;
                ...
                if ($list->getEan() == $xmlEan) {
                    ...
                    foreach ($result->ARTICLE_FEATURES as $productFeatures){
                        foreach ($productFeatures as $productFeature){
                            if($productFeature->FNAME == "-"){
                                $others .= $productFeature->FVALUE . "<br>";
                            }
                        }
                    }

                    $tehnicalData = null;
                    $tehnicalData .= "<p><strong>Technische Daten:</strong></p>";
                    $tehnicalData .= "<ul>";
                    foreach ($result->ARTICLE_FEATURES->FEATURE as $tehnical){
                        $tehnicalData .= "<li>" . $tehnical->FNAME . " " . $tehnical->FVALUE . " " . $tehnical->FUNIT . "</li>";
                    }
                    $tehnicalData .= "</ul>";
                    ...
                    foreach ($result->ARTICLE_FEATURES as $productPackages) {
                        foreach ($productPackages as $productPackage) {
                            ...
                            if ($productPackage->FNAME == "PackagingLength") {
                                $dimensionLength = $productPackage->FVALUE * 100;
                            }
                            if ($productPackage->FNAME == "PackagingWidth") {
                                $dimensionWidth = $productPackage->FVALUE * 100;
                            }
                            if ($productPackage->FNAME == "PackagingHeight") {
                                $dimensionHeight = $productPackage->FVALUE * 100;
                            }
                        }
                    }
                    ...
                    if ($tehnicalData == null) {
                        $tehnicalData = "&nbsp;";
                    }
                    $xmlTechnicalData .= $others;
                    $xmlTechnicalData .= $tehnicalData;
