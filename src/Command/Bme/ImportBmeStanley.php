...
class ImportBmeStanley extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importStanley';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeStanley.xml', 0, TRUE);
        $counter = 0;

        foreach ($xml->T_NEW_CATALOG->PRODUCT as $result) {
            if ($counter <= 10) {

                $xmlEan = $result->PRODUCT_DETAILS->EAN;
                ...
                foreach ($eanList as $list) {
                    if ($list->getEan() == $xmlEan) {
                        ...
                        foreach ($result->PRODUCT_FEATURES->FEATURE as $item){
                            $technicalData .= "<li>" .  $item->FDESCR . " " . $item->FVALUE . " " . $item->FUNIT . "</li>";
                        }
                        $technicalData .= "</ul>";
                        ...
                        if($checkFirstSixCharacters == "STANLEY"){
                            $shortDescriptionVar = substr($shortDescriptionVar, 7);
                            $shortDescriptionVar = "STANLEY " . $shortDescriptionVar;
                        }else{
                            $shortDescriptionVar = "STANLEY " . $shortDescriptionVar;
                        }
                        ...
                        foreach ($result->PRODUCT_FEATURES->FEATURE as $productPackage) {
                            $blockElementsTechnical[] = [
                                "name" => new BlockElement('name', 'input', strval($productPackage->FDESCR)),
