...
class ImportBmeHazet extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importHazet';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeHazet.xml', 0, TRUE);
        $counter = 0;
                    ...
                    if ($list->getEan() == $xmlEan) {
                        ...
                        foreach ($result->ARTICLE_FEATURES as $productPackages){
                            foreach ($productPackages as $productPackage){
                                if($productPackage->FNAME != "Produktempfehlung" && $productPackage->FNAME != "Branche" && $technicalDataCounter > 1){
                                    $technicalData .= "<li>" . $productPackage->FNAME . " " . $productPackage->FVALUE . " " . $productPackage->FUNIT . "</li>";
                                }
                                $technicalDataCounter++;
                            }
                        ...
                        $checkFirstSixCharacters = substr($shortDescriptionVar, 0, 5);
                        if($checkFirstSixCharacters == "Hazet"){
                            $shortDescriptionVar = substr($shortDescriptionVar, 5);
                            $shortDescriptionVar = "Hazet " . $shortDescriptionVar;
                        }else{
                            $shortDescriptionVar = "Hazet " . $shortDescriptionVar;
                        }
                        ...
                        foreach ($result->ARTICLE_FEATURES as $articleFeatures) {
                            ...
                            foreach ($articleFeatures as $productPackage) {
                                if($productPackage->FNAME != "Produktempfehlung" && $productPackage->FNAME != "Branche" && $technicalCounter > 1){
                                    $blockElementsTechnical[] = [
