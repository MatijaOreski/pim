...
class ImportBmeKarcher extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importKarcher';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeKarcher.xml', 0, TRUE);
        $counter = 0;
                ...
                foreach ($eanList as $list) {
                    if ($list->getEan() == $xmlEan) {
                        ...
                        foreach ($result->ARTICLE_FEATURES as $items){
                            ...
                            if($items->REFERENCE_FEATURE_SYSTEM_NAME == "udf_NMSCOPEOFSUPPLY-1.0"){
                                $lieferumfang .= "<p><strong>Lieferumfang:</strong></p>";
                                $lieferumfang .= "<ul>";
                                foreach ($items->FEATURE as $item){
                                    $lieferumfang .= "<li>" . $item->FVALUE . "</li>";
                                }
                                $lieferumfang .= "</ul>";
                            }
                            if($items->REFERENCE_FEATURE_SYSTEM_NAME == "udf_NMBENEFITS-1.0"){
                                $produktvorteile .= "<p><strong>Produktvorteile:</strong></p>";
                                $produktvorteile .= "<ul>";
                                foreach ($items->FEATURE as $item){
                                    $produktvorteile .= "<li>" . $item->FVALUE . "</li>";
                                }
                                $produktvorteile .= "</ul>";
                            }
                            if($items->REFERENCE_FEATURE_SYSTEM_NAME == "udf_NMTOPFEATURES-1.0"){
                                $topFeature .= "<p><strong>Top Feature:</strong></p>";
                                $topFeature .= "<ul>";
                                foreach ($items->FEATURE as $item){
                                    $topFeature .= "<li>" . $item->FVALUE . "</li>";
                                }
                                $topFeature .= "</ul>";
                            }
                        }
                        ...
                        $xmlTechnicalData .= $produktvorteile;
                        $xmlTechnicalData .= $topFeature;
                        ...
                        foreach ($result->ARTICLE_FEATURES as $productPackages) {
                            if($productPackages->REFERENCE_FEATURE_GROUP_NAME == "Ergänzende Logistische Angaben"){
                                foreach ($productPackages->FEATURE as $productPackage){
                                    if($productPackage->FNAME == "Gewicht (mit Zubehör)"){
                                        $nettoWeight = $productPackage->FVALUE;
                                    }
                                    if($productPackage->FNAME == "Länge des Produktes"){
                                        $dimensionLength = $productPackage->FVALUE;
                                    }
                                    if($productPackage->FNAME == "Breite des Produktes"){
                                        $dimensionWidth = $productPackage->FVALUE;
                                    }
                                    if($productPackage->FNAME == "Höhe des Produktes"){
                        ...
                        foreach ($result->MIME_INFO as $articleImages) {
                            foreach ($articleImages as $articleImage) {
                                if ($articleImage->MIME_TYPE == "image/jpeg" && $articleImage->MIME_DESCR == "Product Image" && $articleImage->MIME_PURPOSE == "normal") {
