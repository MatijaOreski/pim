...
class ImportBmeBlackDecker extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'importBlackDecker';
    ...
    private function importNewAssets($path)
    {
        ...
        $xml = new SimpleXMLElement('BmeBlackDecker.xml', 0, TRUE);
        $counter = 0;
            ...
            if ($counter == 0) {
                ...
                /*foreach ($result->ARTICLE_FEATURES as $articles) {
                    if ($articles->REFERENCE_FEATURE_GROUP_NAME == "Technische Daten") {
                        foreach ($articles->FEATURE as $article) {
                            if(strpos(trim($article->FVALUE), " ") != false){
                                echo $article->FVALUE . "\n";
                            }
                ...
                foreach ($eanList as $list) {
                    if ($list->getEan() == $xmlEan) {
                        ...
                        foreach ($result->ARTICLE_FEATURES as $articles){
                            if($articles->REFERENCE_FEATURE_GROUP_NAME == "Lieferumfang"){
                                $lieferumfang .= "<p>Lieferumfang</p>";
                                $lieferumfang .= "<ul>";
                                foreach ($articles->FEATURE as $article){
                                    $lieferumfang .= "<li>" . $article->FVALUE . "</li>";
                                }
                                $lieferumfang .= "</ul>";
                            }
                            if($articles->REFERENCE_FEATURE_GROUP_NAME == "Technische Daten"){
                                ...
                                $technicalData .= "<ul>";
                                foreach ($articles->FEATURE as $article){
                                    $technicalData .= "<li>" . $article->FVALUE . "</li>";
                                }
                                $technicalData .= "</ul>";
                            }
                            if($articles->REFERENCE_FEATURE_GROUP_NAME == "Produktstärken"){
                                $produktstarken .= "<p>Produktstärken</p>";
                                $produktstarken .= "<ul>";
                                foreach ($articles->FEATURE as $article){
                                    $produktstarken .= "<li>" . $article->FVALUE . "</li>";
                                }
                                $produktstarken .= "</ul>";
                            }
                            if($articles->REFERENCE_FEATURE_GROUP_NAME == "Anwendungsbeispiele"){
                                $anwendungsbeispiele .= "<p>Lieferumfang</p>";
                                $anwendungsbeispiele .= "<ul>";
                                foreach ($articles->FEATURE as $article){
                                    $anwendungsbeispiele .= "<li>" . $article->FVALUE . "</li>";
                                }
                                $anwendungsbeispiele .= "</ul>";
                            }
                        ...
                        $xmlTechnicalData .= $produktstarken;
                        $xmlTechnicalData .= $anwendungsbeispiele;
