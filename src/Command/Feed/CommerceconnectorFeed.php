...
class CommerceconnectorFeed extends Command
{
// the name of the command (the part after "bin/console")
    protected static $defaultName = 'mts:commerceconnectorFeeds';
    ...
    private function importNewAssets()
    {
        ...
        foreach ($productDataObjectArray as $productDataObjectResult){
            if($productDataObjectResult->getPublished() == true){
                $productsFeedArray[] = [
                    ...
                    //"Deeplink" => $productDataObjectResult->getProductsName(),
                    "Kurztext" => $productDataObjectResult->getManufacturer()->getName(),
                    ...
                    "ean" => $productDataObjectResult->getEan(),
                    //"direktkauf" => $productDataObjectResult->getPriceBrutto(),
                    //"checkoutLimitPerPeriod" => $productDataObjectResult->getWeightNetto(),
                    //"quantityPerPeriod" => $productDataObjectResult->getEan(),
                    //"minimumPrice" => $productDataObjectResult->getWeightNetto(),
                    //"fulfillmentType" => $productDataObjectResult->getEan()
                ];
        ...
        $headers = array("prodductId", "name", "brand", "offreid", "category", "description", "price", "image", "url", "availability", "Versandkosten", "Vorkasse", "Versandkosten", "Kreditkarte", "Versandkosten", "PayPal", "weight", "ean", "direktkauf", "checkoutLimitPerPeriod", "quantityPerPeriod", "minimumPrice", "fulfillmentType");
        $fh = fopen("public/var/feeds/idealo-de-temp.csv", "w");
        fputcsv($fh, $headers, "\t");
        ...
        copy("public/var/feeds/idealo-de-temp.csv", "public/var/feeds/idealo-de.csv");
